...
for nel_idx = 1:subplot_length
    ...
    for i = 1:nel
        ...
        sigma(i) = sigma(i) / 1e6;  % Convertendo para MPa
    end
    ...
    sigma_analitica = sigma_analitica / 1e6;  % Convertendo para MPa
    ...
    hold on;
    plot(x, sigma_analitica, '-r');
    title(sprintf('Tensão Interna (%d elementos)', nel));
    xlabel('Posição (m)');
    ylabel('Tensão Interna (MPa)');
    grid on;
end
...
legend('show');
grid on;
title('Erro Percentual no Deslocamento');
