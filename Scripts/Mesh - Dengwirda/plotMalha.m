...
vnos=[1:nnos]';
nnel=size(inci(:,1:end),2);      %Numero de ntoseleme por elemento
dimension=size(coord,2); %Dimensao da malha
...
if dimension==2            %For 2D plots
    for iel=1:nel
        for i=1:nnel
            X(i,iel)=coord(inci(iel,i),1);
            Y(i,iel)=coord(inci(iel,i),2);
        end
    end
    ...
    switch index
        ...
        axis equal
        %Numeracao
        text(coord(:,1),coord(:,2),int2str(vnos(:,1)),'fontsize',8,'color','k');
        for i = 1:nel
